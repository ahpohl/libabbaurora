...
void ABBAurora::SetLogLevel(const unsigned char &log_level) { Log = log_level; }
...
void ABBAurora::SetAddress(const unsigned char &addr) { Address = addr; }

unsigned char ABBAurora::GetAddress(void) const { return Address; }

std::string ABBAurora::GetErrorMessage(void) const { return ErrorMessage; }

bool ABBAurora::Send(SendCommandEnum cmd, uint8_t b2, uint8_t b3, uint8_t b4,
  ...
  if (!(Serial->Word(ReceiveData[7], ReceiveData[6]) ==
        Serial->Crc16(ReceiveData, 0, 6))) {
    ErrorMessage = "Received serial package with CRC mismatch";
    Serial->Flush();
    return false;
  }
  if ((cmd != SendCommandEnum::PN_READING) &&
      (cmd != SendCommandEnum::SERIAL_NUMBER_READING) && ReceiveData[0]) {
    ErrorMessage = std::string("Transmission error: ") +
                   ABBAuroraStrings::TransmissionState(ReceiveData[0]) + " (" +
...
bool ABBAurora::ReadDspValue(float &value, const DspValueEnum &type,
                             const DspGlobalEnum &global) {
  if (!Send(SendCommandEnum::MEASURE_REQUEST_DSP, static_cast<uint8_t>(type),
            static_cast<uint8_t>(global), 0, 0, 0, 0)) {
    return false;
  }
  if (ReceiveData[1] != 6) // global state "Run"
  {
    ErrorMessage =
        std::string("Warning DSP value not trusted: Inverter state \"") +
        ABBAuroraStrings::GlobalState(ReceiveData[1]) + "\" (" +
        std::to_string(ReceiveData[1]) + ")";
    return false;
  }
  uint8_t b[] = {ReceiveData[5], ReceiveData[4], ReceiveData[3],
                 ReceiveData[2]};
...
bool ABBAurora::ReadCumulatedEnergy(float &cum_energy,
  ...
  if (!Send(SendCommandEnum::CUMULATED_ENERGY_READINGS,
            static_cast<uint8_t>(period), 0, 0, 0, 0, 0)) {
    return false;
  }
  if (ReceiveData[1] != 6) // global state "Run"
  {
    ErrorMessage =
        std::string("Warning cumulated energy not trusted: Inverter state \"") +
        ABBAuroraStrings::GlobalState(ReceiveData[1]) + "\" (" +
        std::to_string(ReceiveData[1]) + ")";
    return false;
  }
